...
class TestAIWorkOrderCreation:
    ...
    def test_created_wo_appears_in_work_orders_list(self):
        ...
        # Note: source field may not be exposed in response model, but is stored in DB
        
        print(f"Verified WO #{wo_data.get('numero')}: statut={wo_data.get('statut')}, categorie={wo_data.get('categorie')}")
