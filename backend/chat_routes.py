...
async def transfer_to_improvement(
    ...
    try:
        ...
        # Vérifier que l'amélioration existe (chercher par id UUID, pas _id)
        improvement = await db.improvements.find_one({"id": improvement_id})
        if not improvement:
        ...
        # Générer un nom unique pour éviter les conflits
        unique_filename = f"{uuid_mod.uuid4().hex[:8]}_{attachment.get('filename', attachment.get('original_filename', 'file'))}"
        new_file_path = os.path.join(improvements_dir, unique_filename)
        shutil.copy2(source_path, new_file_path)
        
        # Ajouter à l'amélioration (chercher par id UUID)
        new_attachment = {
            "id": str(uuid_mod.uuid4()),
            "filename": attachment.get("original_filename") or attachment.get("filename"),
            "path": new_file_path,
        ...
        logger.info(f"Fichier transféré vers amélioration {improvement_id}: {unique_filename}")
        return {"success": True, "message": "Fichier transféré vers l'amélioration"}
    
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Erreur transfert vers amélioration: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Erreur lors du transfert: {str(e)}")
