...
###############################################################################
# ETAPE 1 : Prerequis Tailscale (instructions admin console)
###############################################################################

echo "------------------------------------------------------------------"
echo "  Etape 1/6 : Prerequis Tailscale"
echo "------------------------------------------------------------------"
echo ""
...
echo "     Admin Console > Access Controls > ajouter dans la config :"
echo ""
echo '     "nodeAttrs": ['
echo '       {'
...
###############################################################################
# ETAPE 2 : Installation de Tailscale
###############################################################################

echo ""
echo "------------------------------------------------------------------"
echo "  Etape 2/6 : Installation de Tailscale"
echo "------------------------------------------------------------------"
echo ""
...
###############################################################################
# ETAPE 3 : Connexion a Tailscale
###############################################################################

echo ""
echo "------------------------------------------------------------------"
echo "  Etape 3/6 : Connexion a votre compte Tailscale"
echo "------------------------------------------------------------------"
echo ""
...
###############################################################################
# ETAPE 4 : Configuration Nginx (HTTP simple, sans SSL)
###############################################################################

echo ""
echo "------------------------------------------------------------------"
echo "  Etape 4/6 : Reconfiguration Nginx"
echo "------------------------------------------------------------------"
echo ""
...
###############################################################################
# ETAPE 5 : Activation de Tailscale Funnel
###############################################################################

echo ""
echo "------------------------------------------------------------------"
echo "  Etape 5/6 : Activation de Tailscale Funnel"
echo "------------------------------------------------------------------"
echo ""
...
###############################################################################
# ETAPE 6 : Mise a jour du .env backend
###############################################################################

echo ""
echo "------------------------------------------------------------------"
echo "  Etape 6/6 : Mise a jour de la configuration backend"
echo "------------------------------------------------------------------"
echo ""
...
if grep -q "^GOOGLE_DRIVE_REDIRECT_URI=" "$BACKEND_ENV"; then
    ...
    echo "  Ancienne : https://gmaoiris.duckdns.org/api/backup/drive/callback"
...
if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "304" ]; then
    ...
    warn "Impossible de se connecter. Le Funnel peut prendre quelques secondes a se propager."
...
if [[ "$CLEANUP_LE" =~ ^[OoYy]$ ]]; then
    # Desactiver le timer de renouvellement
...
echo "  SSL                : Gere automatiquement par Tailscale"
echo "  Renouvellement     : Automatique (Tailscale)"
echo "  Pare-feu           : Aucun port a ouvrir"
echo ""

if grep -q "^GOOGLE_DRIVE_REDIRECT_URI=" "$BACKEND_ENV"; then
    echo "  Google Drive       : URI mise a jour"
    echo ""
...
echo "------------------------------------------------------------------"
echo ""
echo "  Verifier le statut Tailscale :"
echo "    tailscale status"
echo ""
echo "  Verifier le statut Funnel :"
echo "    tailscale funnel status"
echo ""
echo "  Desactiver Funnel temporairement :"
echo "    tailscale funnel off"
echo ""
echo "  Reactiver Funnel :"
