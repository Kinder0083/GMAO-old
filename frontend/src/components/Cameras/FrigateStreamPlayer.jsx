 ...
 * Le backend récupère les frames depuis go2rtc et les transmet au client
...
const FrigateStreamPlayer = ({ 
  ...
  // Construire l'URL du stream via le proxy backend
  const getStreamUrl = () => {
    ...
    >
      ...
      <CardContent className="p-0 relative">
        <div className={`relative bg-gray-900 ${isFullscreen ? 'h-screen' : 'aspect-video'}`}>
          {/* Stream MJPEG via img tag (proxy backend) */}
          ...
          {status === 'connected' && (
            <div className="absolute bottom-2 left-2 px-2 py-1 bg-black/60 rounded text-white text-xs flex items-center gap-2">
              ...
              {isH264 && <span className="text-green-400">H264</span>}
            </div>
          )}
        </div>
      </CardContent>
