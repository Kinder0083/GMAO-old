...
import { Canvas, Rect, Circle as FabricCircle, IText, FabricObject, PencilBrush, util } from 'fabric';
...
const WhiteboardPage = () => {
  ...
  useEffect(() => {
    ...
    if (container1Ref.current && !canvas1 && !canvas1InitializedRef.current) {
      ...
      const existingCanvasElement = document.getElementById('canvas-board_1');
    ...
    if (container2Ref.current && !canvas2 && !canvas2InitializedRef.current) {
      ...
      canvas2InitializedRef.current = true;
  ...
  useEffect(() => {
    ...
    const handleTextChanged = async (e) => {
      ...
      const obj = e.target;
      if (obj.id) {
        console.log('[Canvas board_1] Texte modifié:', obj.id);
        await updateObjectAPI('board_1', obj);
      }
    };
    ...
    canvas1.on('text:changed', handleTextChanged);
    ...
    return () => {
      ...
      canvas1.off('text:changed', handleTextChanged);
    };
